module overmind::bingo_core {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        assert_admin(signer_address);
        // TODO: Create a bingo resource account
        let (resource_signer, resource_cap) = account::create_resource_account(admin, BINGO_SEED);
        ...
        move_to<Bingo>(&resource_signer,bingo_g);
    }
    ...
    */
    public entry fun create_game(
        ...
        let signer_address = signer::address_of(admin);
        assert_bingo_initialized(signer_address);

        // TODO: Assert that the game name is not taken
        let state_res = borrow_global<State>(signer_address);
        let bingo_g = borrow_global_mut<Bingo>(state_res.bingo);
        assert_game_name_not_taken(&bingo_g.games,&game_name);
        ...
        simple_map::add(&mut bingo_g.games, game_name, new_game);

        // TODO: Emit CreateGameEvent event
        event::emit_event<CreateGameEvent>(
            &mut bingo_g.create_game_events,
            overmind::bingo_events::new_create_game_event(game_name,entry_fee,start_timestamp,timestamp::now_seconds()),
        );
    }
    ...
    */
    public entry fun insert_number(admin: &signer, game_name: String, number: u8) acquires State, Bingo {
        ...
        let state_res = borrow_global<State>(signer_address);
        let bingo_g = borrow_global_mut<Bingo>(state_res.bingo);
        assert_game_exists(&bingo_g.games,&game_name);

        // TODO: Assert that the game already started
        let cur_game = simple_map::borrow_mut(&mut bingo_g.games,&game_name);
        assert_game_already_stared(cur_game.start_timestamp);

        // TODO: Assert that the drawn number is not a duplicate
        assert_number_not_duplicated(&cur_game.drawn_numbers, &number);

        // TODO: Add the drawn number to game's drawn numbers
        vector::push_back(&mut cur_game.drawn_numbers,number);

        // TODO: Emit InsertNumberEvent event
        event::emit_event<InsertNumberEvent>(
            &mut cur_game.insert_number_events,
            overmind::bingo_events::new_inser_number_event(game_name,number,timestamp::now_seconds()),
        );
    }
    ...
    */
    public entry fun join_game(player: &signer, game_name: String, numbers: vector<vector<u8>>) acquires State, Bingo {
        // TODO: Assert that bingo is initialized
        assert_bingo_initialized(@overmind);
        ...
        // TODO: Assert that the game exists
        let state_res = borrow_global<State>(@overmind);
        let bingo_g = borrow_global_mut<Bingo>(state_res.bingo);
        assert_game_exists(&bingo_g.games,&game_name);

        // TODO: Assert that the game has not started yet
        let cur_game = simple_map::borrow_mut(&mut bingo_g.games,&game_name);
        assert_game_not_started(cur_game.start_timestamp);

        // TODO: Assert that the player has enough APT to join the game
        let player_address = signer::address_of(player);
        assert_suffiecient_funds_to_join(player_address, cur_game.entry_fee);

        // TODO: Assert that the player has not joined the game yet
        assert_player_not_joined_yet(&cur_game.players, &player_address);

        // TODO: Add the player to the game's list of players
        let player_numbers = vector::empty();
        let i = 0;
        while(i < 5){
            let row_nums = vector::empty();
            let row = *vector::borrow(&numbers,i);
            vector::append(&mut row_nums, row);
            vector::push_back(&mut player_numbers,row);
            i = i + 1;
        };
        simple_map::add(&mut cur_game.players, player_address, player_numbers);

        // TODO: Transfer entry fee from the player to the bingo PDA
        coin::transfer<AptosCoin>(player,state_res.bingo,cur_game.entry_fee);

        // TODO: Emit JoinGameEvent event
        event::emit_event<JoinGameEvent>(
            &mut cur_game.join_game_events,
            overmind::bingo_events::new_join_game_event(game_name,player_address,player_numbers,timestamp::now_seconds()),
        );
    }
    ...
    */
    public entry fun bingo(player: &signer, game_name: String) acquires State, Bingo {
        ...
        let cur_game = simple_map::borrow_mut(&mut bingo_g.games,&game_name);
        assert_game_not_finished(cur_game.is_finished);

        // TODO: Assert that the player joined the game
        let player_address = signer::address_of(player);
        assert_player_joined(&cur_game.players, &player_address);

        // TODO: Assert that the player has bingo
        let player_numbers = *simple_map::borrow(&cur_game.players,&player_address);
        assert_player_has_bingo(&cur_game.drawn_numbers, player_numbers);
        ...
        let bingo_signer = account::create_signer_with_capability(&bingo_g.cap);
        coin::transfer<AptosCoin>(&bingo_signer,player_address,cur_game.entry_fee*simple_map::length(&cur_game.players));

        // TODO: Emit BingoEvent event
        event::emit_event<BingoEvent>(
            &mut cur_game.bingo_events,
            overmind::bingo_events::new_bingo_event(game_name,player_address,timestamp::now_seconds()),
        );
    }
    ...
    */
    public entry fun cancel_game(admin: &signer, game_name: String) acquires State, Bingo {
        ...
        let i = 0;
        let bingo_signer = account::create_signer_with_capability(&bingo_g.cap);
        let (players, _cards) = simple_map::to_vec_pair(cur_game.players);
        while (i < vector::length(&players)){
            coin::transfer<AptosCoin>(&bingo_signer,*vector::borrow(&players,i),cur_game.entry_fee);
            i = i + 1;
        };

        // TODO: Emit CancelGameEvent event
        event::emit_event<CancelGameEvent>(
            &mut bingo_g.cancel_game_events,
            overmind::bingo_events::new_cance_game_event(game_name,timestamp::now_seconds()),
        );
    }
    ...
    */
    fun check_player_numbers(drawn_numbers: &vector<u8>, player_numbers: vector<vector<u8>>): bool {
        ...
        let new_player_numbers = vector::empty();
        let i = 0;
        while (i < vector::length(&player_numbers)){
            let j = 0;
            let col = *vector::borrow_mut(&mut player_numbers,(i as u64));
            let new_col = vector::empty();
            while(j < vector::length(&player_numbers)){
                let num = *vector::borrow_mut(&mut col, (j as u64));
                if(vector::contains(drawn_numbers, &num) || num == 0){
                    vector::push_back(&mut new_col, option::none());
                }else{
                    vector::push_back(&mut new_col, option::some(100));
                };
                j = j + 1;
            };
            vector::push_back(&mut new_player_numbers, new_col);
            i = i + 1;
        };

        // TODO: Call check_columns, check_diagonals and check_rows and return true if any of those returns true
        let got_bingo = false;
        if(check_columns(&new_player_numbers) || check_rows(&new_player_numbers) || check_diagonals(&new_player_numbers)){
            got_bingo = true;
        };
        return got_bingo
    }
    ...
    */
    inline fun check_columns(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        while (i < vector::length(player_numbers)){
            let j = 0;
            let five_none = 0;
            let col = *vector::borrow(player_numbers,(i as u64));
            while(j < vector::length(player_numbers)){
                let num = *vector::borrow(&col, (j as u64));
                if(option::is_none(&num)){
                    five_none = five_none + 1;
                    j = j + 1;
                }else{
                    break
                };
    ...
    */
    inline fun check_rows(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        let got_bingo = false;
        let i = 0;
        while (i < vector::length(player_numbers) && got_bingo == false){
            let j = 0;
            let five_none = 0;            
            while(j < vector::length(player_numbers)){
                let col = *vector::borrow(player_numbers,(j as u64));
                let num = *vector::borrow(&col, (i as u64));
                if(option::is_none(&num)){
                    five_none = five_none + 1;
                    j = j + 1;
                }else{
    ...
    */
    ...
    inline fun check_diagonals(player_numbers: &vector<vector<Option<u8>>>): bool {
        ...
        let got_bingo = false;
        if(return_vv_num(player_numbers,0,0) && return_vv_num(player_numbers,1,1) && return_vv_num(player_numbers,2,2) && return_vv_num(player_numbers,3,3) && return_vv_num(player_numbers,4,4)){
            got_bingo = true;
        };
        if(return_vv_num(player_numbers,4,0) && return_vv_num(player_numbers,3,1) && return_vv_num(player_numbers,2,2) && return_vv_num(player_numbers,1,3) && return_vv_num(player_numbers,0,4)){
            got_bingo = true;
        };
        got_bingo
    }
    ...
    inline fun assert_admin(admin: address) {
        ...
        assert!(admin == @overmind,SIGNER_NOT_ADMIN);
    }
    ...
    inline fun assert_bingo_initialized(admin: address) acquires State {
        ...
        let state_res = borrow_global<State>(admin);
        assert!(exists<Bingo>(state_res.bingo),BINGO_NOT_INITIALIZED);
    }

    inline fun
    assert_game_name_not_taken(games: &SimpleMap<String, Game>, game_name: &String) {
    ...
    inline fun assert_correct_amount_of_picked_numbers(picked_numbers: &vector<vector<u8>>) {
        ...
        let column_size_correct = true;
        let row_size = vector::length(picked_numbers);
        let i = 0;
        while(i < row_size){
            let col = *vector::borrow(picked_numbers,(i as u64));
            if(vector::length(&col) != 5){
                column_size_correct = false;
                break
            };
            i = i + 1;
        };        
        assert!(row_size == 5,INVALID_AMOUNT_OF_COLUMNS_IN_PICKED_NUMBERS);
        assert!(column_size_correct,INVALID_AMOUNT_OF_NUMBERS_IN_COLUMN);
    }
    ...
    inline fun assert_numbers_are_picked_correctly(picked_numbers: &vector<vector<u8>>) {
        ...
        assert!(check_column_num_valid(vector::borrow(picked_numbers,0),1,15),COLUMN_HAS_INVALID_NUMBER);
        //      2) The second column must consist of numbers from a range of <16; 30>
        assert!(check_column_num_valid(vector::borrow(picked_numbers,1),16,30),COLUMN_HAS_INVALID_NUMBER);
        //      3) The third column must consist of numbers from a range of <31; 45>
        assert!(goddamn_column_3_check(vector::borrow(picked_numbers,2),31,45),COLUMN_HAS_INVALID_NUMBER);
        //      4) The fourth column must consist of numbers from a range of <46; 60>
        assert!(check_column_num_valid(vector::borrow(picked_numbers,3),46,60),COLUMN_HAS_INVALID_NUMBER);
        //      5) The fifth column must consist of numbers from a range of <61; 75>
        assert!(check_column_num_valid(vector::borrow(picked_numbers,4),61,75),COLUMN_HAS_INVALID_NUMBER);
    ...
    fun return_vv_num(player_numbers: &vector<vector<Option<u8>>>, i: u8, j: u8): bool{
        let col = *vector::borrow(player_numbers,(j as u64));
        let num = *vector::borrow(&col, (i as u64));
        if(option::is_none(&num)){
            return true
    ...
    fun check_column_num_valid(column: &vector<u8>, low: u8, high: u8): bool{
        let i = 0;
        let check_valid = true;
        while(i < vector::length(column)){
            let num = *vector::borrow(column, (i as u64));
            if(num < low || num > high){
                check_valid = false;
                break
            };
    ...
    fun goddamn_column_3_check(column: &vector<u8>, low: u8, high: u8): bool{
       let i = 0;
        let check_valid = true;
        while(i < vector::length(column)){
            let num = *vector::borrow(column, (i as u64));
